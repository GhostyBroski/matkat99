...
async function getJson(endpoint) {
  ...
  const apiKey = "K1M8FOIkUGervBaRbvTWq8yudg6GYBluIoec0CFH";
...
async function renderClimbingList() {
  ...
  const climbingActivity = data.data.find(activity => activity.name.toLowerCase() === "climbing")

  const parks = climbingActivity ? climbingActivity.parks : []
